...
const Cart = () => {
  ...
  useEffect(() => {
    const cartLocal = JSON.parse(localStorage.getItem('__next__cart01__sanj'))
    if(cartLocal && cartLocal.length > 0){
      ...
      const updateCart = async () => {
        for (const item of cartLocal){
          ...
          if (!res || !res.product) {
            console.error(`Product not found for item with _id: ${item._id}`);
            continue; // Skip to the next item if the product is undefined
          }
